...
CellData::CellData(double d, double i, unsigned int sx, unsigned int sy)
  : distance(d), index(i), sx(sx), sy(sy) {}
...
inline double MapInflationTool::distanceLookup(int mx, int my,
    int src_x, int src_y) {
  unsigned int dx = abs(mx - src_x);
  unsigned int dy = abs(my - src_y);
  if ( dx > mCellInflationRadius + 1 || dy > mCellInflationRadius + 1 ) {
    ROS_ERROR(
        "Error in distanceLookup! Asked for (%d, %d), but CellInflationRadius is %d!",
        dx, dy, mCellInflationRadius);
    return 50.0;
  }
...
inline signed char MapInflationTool::costLookup(int mx, int my,
    int src_x, int src_y) {
  unsigned int dx = abs(mx - src_x);
  unsigned int dy = abs(my - src_y);
  if ( dx > mCellInflationRadius + 1 || dy > mCellInflationRadius + 1 ) {
    ROS_ERROR("Error in costLookup! Asked for (%d, %d), but CellInflationRadius is %d!",
        dx, dy, mCellInflationRadius);
    return 50.0;
  }
...
void MapInflationTool::enqueueObstacle(unsigned int index,
  ...
  if ( distance == 50 )
    ROS_INFO("Tried to add cell (%u, %u) -> (%u, %u) to inflation queue!",
        sx, sy, mx, my);
