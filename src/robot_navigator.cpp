...
void RobotNavigator::receiveGetMapGoal(const nearest_frontier_planner::GetFirstMapGoal::ConstPtr &goal)
{
	...
	nearest_frontier_planner::GetFirstMapFeedback f;
...
void RobotNavigator::receiveLocalizeGoal(const nearest_frontier_planner::LocalizeGoal::ConstPtr &goal)
{
	if(mStatus != NAV_ST_IDLE)
	{
	...
	nearest_frontier_planner::LocalizeFeedback f;
...
void RobotNavigator::receiveMoveGoal(const nearest_frontier_planner::MoveToPosition2DGoal::ConstPtr &goal)
{
	if(mStatus != NAV_ST_IDLE)
	{
		ROS_WARN("Navigator is busy!");
	...
	while(true)
	{
		...
		if(cycle%10 == 0)
		{
			nearest_frontier_planner::MoveToPosition2DFeedback fb;
	...
	nearest_frontier_planner::MoveToPosition2DResult r;
...
void RobotNavigator::receiveExploreGoal(const nearest_frontier_planner::ExploreGoal::ConstPtr &goal)
{
	...
	while(true)
	{
		...
		if(reCheck || nearGoal)
		{
			...
			if(preparePlan())
			{
				...
				case EXPL_FINISHED:
					...
					{
						nearest_frontier_planner::ExploreResult r;
						r.final_pose.x = mCurrentPositionX;
						r.final_pose.y = mCurrentPositionY;
						r.final_pose.theta = mCurrentDirection;
		...
		if(mStatus == NAV_ST_EXPLORING)
		{
			...
			if(cycle%10 == 0)
			{
				nearest_frontier_planner::ExploreFeedback fb;
				fb.distance = mCurrentPlan[mStartPoint];
